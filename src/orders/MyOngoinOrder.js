...
import { CancelRequestApi } from './CancelRequestApi'
import DropDown from '../public/dropDown/DropDown'
import SVGImage from 'react-native-remote-svg'
import { PayInvoiceApi } from './PayInvoiceApi'
import { StartConfirmApi } from './StartConfirmApi'
import call from 'react-native-phone-call'

export default class MyOngoingOrder extends React.Component {
  ...
  render () {
    ...
    return this.state.requestServiceItem && this.state.providerData ? (
      <View style={{ alignItems: 'stretch' }}>
        ...
        >
            ...
            this.state.requestLocation.data ? (
              <CustomCard height={110} style={[Prolar.style.rtlCol]}>
                ...
                <Text
                  ...
                  style={{ marginRight: 10 * Prolar.size.unit }}
                  label={
                    this.state.requestLocation.data.description.replace(
                      /,/g,
                      '،'
                    ) +
                    '، ' +
          ...
          {order.status === 'در انتظار اجازه شروع' ? (
            <View>
              <Button
                color={Prolar.color.primary}
                width={203}
                height={50}
                onPress={() => {
                  StartConfirmApi(this.props.order.id).then(res => {
              ...
              <Button
                ...
                height={50}
                onPress={() => {
                  CancelRequestApi(this.props.order.id).then(res => {
                    if (res.errors.length === 0) {
                      Prolar.navigator(this.props.activity, 'Profile')()
                      this.setState({ msg: undefined })
                    } else {
          ...
          ) : order.status === 'تکمیل - فاکتور صادر شده' ||
            order.status === 'شروع شده' ? (
              undefined
            ) : (
              <Button
                color={Prolar.color.error}
